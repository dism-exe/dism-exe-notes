...
As mentioned in [000 Setup project & tools and build on a new Linux machine > 2.3.1 Reading/writing to memory as a struct (TODO)](000%20Setup%20project%20&%20tools%20and%20build%20on%20a%20new%20Linux%20machine.md#2-3-1-reading-writing-to-memory-as-a-struct-todo), we are unable to modify memory using C type information because although we document structs they are not present in a format gdb understands.
...
[^link1](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#link1) mentions that NanoBoyadvance is currently the most accurate emulator.

## 2.2 Setting up basic gba project for debug symbol injection

We should be able to clone gba-bootstrap [^link3](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#link3) as our toy project. The next thing is to define a struct and map it to some region in EWRAM like `0x2000000`

````sh
git clone https://github.com/davidgfnet/gba-bootstrap.git
````

Because we already setup arm-none-eabi-xxx in [000 Setup project & tools and build on a new Linux machine > 3.2 arm-none-eabi-gdb requiring libncurses.so.5](000%20Setup%20project%20&%20tools%20and%20build%20on%20a%20new%20Linux%20machine.md#3-2-arm-none-eabi-gdb-requiring-libncurses-so-5) we are able to just go to `template_c` and `make`
...
Setup profiling according to [this tutorial](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#tut1) for flamegraph:
...
Let's try to use pprof. See [instructions](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#3-4-3-pprof-rust-setup-to-create-profile-pb).
...
We have some folds to optimize away. The graph included a [link](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#docs2) to documentation for how to read it.
...
[this forum answer](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#forum1-ans1) has nice recommendations on using radare2 to generate a call graph png from an executable, which could be helpful in tracing problems like this.
...
See [3.1 Revising lexer parsing to include inc files and preserve format](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#3-1-revising-lexer-parsing-to-include-inc-files-and-preserve-format)

We are also seeing panics when [processing lexer files](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#3-2-panics-when-processing-lexer-files). ^spawn-task-182155
...
From [2.5.3.3 Resolving Include keys](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#2-5-3-3-resolving-include-keys).
...
Noticed during [2.5.3.3 Resolving Include keys](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#2-5-3-3-resolving-include-keys). [^spawn-task-182155](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#spawn-task-182155).
...
[Task](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#3-3-get-rs-repro-regex-string-tester-to-work). ^spawn-task-072145

2025-08-07 Wk 32 Thu - 23:40

We're unable to compile the above printed regex with rs_repro repro004. [Issue](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#4-6-unable-to-compile-regex-for-vscode-cpp-string-regex-due-to-repetition-quantifier). ^spawn-issue-072345
...
From [^spawn-task-072145](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#spawn-task-072145).
...
We need to import [lan-rs-common](https://github.com/LanHikari22/lan_rs_common) but it allows different feature configurations. And each repro should be minimial with the feature set it requires. For example for regex, as of now, no features are required. [HowTo](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#5-2-specify-multiple-optional-dependencies-with-different-names-to-be-used-by-features-in-cargo-toml-for-rust). ^spawn-howto-072236
...
Let's test for the regex string  `"([^"\\]*(\\.[^"\\]*)*)"\s*` But this has quotes in it and is tricky to put as an argument. [HowTo](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#5-3-specify-strings-that-use-all-single-and-double-quotes-in-their-content-in-shell).  ^spawn-howto-073221
...
Based on instructions in [^tut1](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#tut1) we encounter the following error:
...
[pprof documentation](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#docs1) offers different instructions, maybe due to version changes.

For the official documentation [code](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#3-4-2-pprof-rust-setup-to-create-profile-pb) , some changes were necessary:
...
Via [this diagnostic log](../../llm/2025/001%20General%20Assist%20Archive.md#1-no-method-pprof-in-report), we need to issue the following corrections:
...
In order to visualize `prof.pb`, we need [pprof](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#link4). Here are the [Installation instructions](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#3-5-pprof-and-go-installation).
...
From [^spawn-issue-072345](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#spawn-issue-072345).
...
From [^spawn-howto-072236](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#spawn-howto-072236).
...
From [^spawn-howto-073221](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#spawn-howto-073221).
...
While looking into [5.2 Specify multiple optional dependencies with different names to be used by features in Cargo.toml for rust](001%20Turn%20EWRAM%20and%20ROM%20Structs%20into%20C%20Structs%20and%20embed%20into%20type%20for%20gdb%20memory%20manipulation.md#5-2-specify-multiple-optional-dependencies-with-different-names-to-be-used-by-features-in-cargo-toml-for-rust),
...
1. https://github.com/AntonioND/gba-bootstrap ^link2
1. https://github.com/davidgfnet/gba-bootstrap ^link3
1. https://markaicode.com/profiling-applications-2025/ ^tut1
1. https://docs.rs/crate/pprof/latest ^docs1
1. https://github.com/google/pprof ^link4
1. https://github.com/tikv/pprof-rs/issues/273 ^issue1
1. https://git.io/JfYMW ^docs2
1. https://reverseengineering.stackexchange.com/questions/16081/how-to-generate-the-call-graph-of-a-binary-file ^forum1
1. https://reverseengineering.stackexchange.com/a/16082 ^forum1-ans1
1. https://book.rada.re/ ^docs3
